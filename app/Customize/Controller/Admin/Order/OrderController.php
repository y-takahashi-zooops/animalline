        ...
        $response->setCallback(function () use ($request, $csvTypeId) {
            ...
            $this->csvExportService->exportData(function ($entity, $csvService) use ($request) {
                ...
                foreach ($OrderItems as $OrderItem) {
                    ...
                    //受注CSV出力の時に関連ブリーダー・保護団体の項目追加
                    //if($csvTypeId == CsvType::CSV_TYPE_ORDER){
                        $Customer = $Order->getCustomer();
                        $incentive_id = intval($Customer->getRegisterId());
                        if($incentive_id == 0){
                            $incentive_id = 95;
                        }
                        $incentive_customer = $this->customerRepository->find($incentive_id);
                        if($incentive_customer->getIsBreeder() == 1){
                            $incentive_org = $this->breedersRepository->find($incentive_id);
                            $ExportCsvRow->setData($incentive_org->getBreederName());
                        }
                        else{
                            $incentive_org = $this->conservationsRepository->find($incentive_id);
                            $ExportCsvRow->setData($incentive_org->getOrganizationName());
                        }
