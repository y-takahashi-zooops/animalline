...
class OrderTypeCustomizer extends AbstractTypeExtension
{
    /**
     * @var OrderRepository
     */
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        $builder->addEventListener(FormEvents::POST_SET_DATA, function (FormEvent $event) {
            ...
            // カートに定期注文が入っている場合、銀行振込の選択肢をフィルターする
            $Cart = $this->cartService->getCart();
            if ($Cart) {
                $CartItems = $Cart->getCartItems();
                foreach ($CartItems as $CartItem) {
                    if ($CartItem->getIsRepeat()) {
                        $Payments = $this->filterSubscriptionPayments($Payments);
                    }
                }
            }
     ...
     * @return Payment[]
     */
    private function filterSubscriptionPayments(ArrayCollection $Payments)
    {
        $PaymentArrays = $Payments->filter(function (Payment $Payment) {

            if ($Payment->getMethod() == '銀行振込') {
                return false;
            }
        ...
        return new ArrayCollection($PaymentArrays);
