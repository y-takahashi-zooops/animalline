...
class DnaQueryService
{
    ...
    public function filterDnaAdmin(array $criteria): array
    {
        ...
        $queryConservation = $this->dnaCheckStatusRepository->createQueryBuilder('dna')
            ...
            ->select('dna.id as dna_id, dna.site_type, cp.id as pet_id, c.id as customer_id, cp.is_active, cp.thumbnail_path, cp.pet_kind, b.breeds_name, dna.check_status, dnah.kit_shipping_date, dna.kit_return_date, dna.check_return_date, dna.file_path, dna.update_date,c.name01,c.name02,dnah.shipping_name,dnah.shipping_pref,dnah.shipping_city,dnah.shipping_address');
        ...
        $queryBreeder = $this->dnaCheckStatusRepository->createQueryBuilder('dna')
            ...
            ->select('dna.id as dna_id, dna.site_type, bp.id as pet_id, c.id as customer_id, bp.is_active, bp.thumbnail_path, bp.pet_kind, b.breeds_name, dna.check_status, dnah.kit_shipping_date, dna.kit_return_date, dna.check_return_date, dna.file_path, dna.update_date,c.name01,c.name02,dnah.shipping_name,dnah.shipping_pref,dnah.shipping_city,dnah.shipping_address,dnah.labo_type');
        if (!empty($customerName)) {
